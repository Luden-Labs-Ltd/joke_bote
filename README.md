...
Добавьте бота в каждую рабочую группу и вызовите там `/startmeet crm/api` от имени администратора. Аргумент `crm/api` необязателен: это название проекта, которое бот сохранит вместе с названием Telegram-группы. Затем бот покажет кнопки для выбора времени и дней недели — расписание хранится отдельно для каждой группы. Для каждого созвона бот создаёт отдельную ссылку Meet, присылает её за 10 минут до выбранного времени и повторно в момент начала. Для отключения используйте `/stopmeet`.
...
В Railway задайте `GITHUB_WEBHOOK_SECRET` (случайный секрет), `GITHUB_COMMIT_CHAT_IDS`, `GITHUB_API_TOKEN` и `GEMINI_API_KEY`. GitHub token должен быть fine-grained с доступом к нужной организации и разрешением **Contents: Read-only** — по нему бот читает diff конкретного коммита, в том числе в приватных репозиториях. Все ИИ-генерации бота работают через Gemini: описания коммитов используют `GEMINI_COMMIT_MODEL` (по умолчанию `gemini-2.5-flash`), а групповые и meeting-сводки — `GEMINI_MODEL` (по умолчанию `gemini-2.5-flash`). Описание коммита принудительно запрашивается на русском, даже если commit message и diff на английском. Если Gemini временно недоступен, бот пришлёт нейтральную русскую подпись со ссылкой на коммит, а в Railway Logs будет сохранён точный ответ Gemini. Затем в настройках организации GitHub создайте Webhook с URL `https://<railway-domain>/webhooks/github`, тем же секретом, форматом `application/json` и единственным событием `Pushes`.
