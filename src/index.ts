...
const persistentStateDirectory = dirname(meetingSubscriptionFile);
const meetingSummaryFile = process.env.MEETING_SUMMARY_FILE?.trim() || "./data/meeting-summaries.json";
const dailySummarySubscriptionFile = process.env.DAILY_SUMMARY_SUBSCRIPTIONS_FILE?.trim() || `${persistentStateDirectory}/daily-summary-subscriptions.json`;
const dailySummaryFile = process.env.DAILY_SUMMARY_FILE?.trim() || `${persistentStateDirectory}/daily-summaries.json`;
...
const openaiCommitModel = process.env.OPENAI_COMMIT_MODEL?.trim() || "gpt-5-mini";
